...
**Hello-World bootstrap.** The repository is the generated VS Code extension scaffold (a single sample command, no OCX-specific behavior yet). The rules layer and tooling baseline are being established first so feature work lands on a solid foundation. Treat any OCX-specific capability below as **roadmap, not present**.
